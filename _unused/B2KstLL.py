...
def fold_P4p(data, costheta_k, costheta_l, phi):
    ...
    data[f"{phi}_P4p"] = np.where(data[phi] < 0, -data[phi], data[phi])
    data[f"{phi}_P4p"] = np.where(
        theta_l > 0.5 * pi, pi - data[f"{phi}_P4p"], data[f"{phi}_P4p"]
    )
    data[f"{costheta_l}_P4p"] = np.where(
        theta_l > 0.5 * pi, np.cos(pi - theta_l), data[costheta_l]
    )

    return zfit.data.Data.from_pandas(
        data[f"{costheta_l}_P4p", f"{costheta_k}_P4p", f"{phi}_P4p"]
        .copy()
        .rename(
            index=str,
            columns={
                f"{costheta_l}_P4p": costheta_l,
...
def fold_P5p(data, costheta_k, costheta_l, phi):
    ...
    data[f"{phi}_P5p"] = np.where(
        data[f"{phi}_P5p"] < 0, -data[f"{phi}_P5p"], data[f"{phi}_P5p"]
    )
    data[f"{costheta_l}_P5p"] = np.where(
        theta_l > 0.5 * pi, np.cos(pi - theta_l), data[costheta_l]
    )

    return zfit.data.Data.from_pandas(
        data[f"{costheta_l}_P5p", f"{costheta_k}_P5p", f"{phi}_P5p"]
        .copy()
        .rename(
            index=str,
            columns={
                f"{costheta_l}_P5p": costheta_l,
...
def fold_P6p(data, costheta_k, costheta_l, phi):
    ...
    data[f"{phi}_P6p"] = np.where(data[phi] > 0.5 * pi, pi - data[phi], data[phi])
    data[f"{phi}_P6p"] = np.where(
        data[f"{phi}_P6p"] < -0.5 * pi, -pi - data[f"{phi}_P6p"], data[f"{phi}_P6p"]
    )
    data[f"{costheta_l}_P6p"] = np.where(
        theta_l > 0.5 * pi, np.cos(pi - theta_l), data[costheta_l]
    )

    return zfit.data.Data.from_pandas(
        data[f"{costheta_l}_P6p", f"{costheta_k}_P6p", f"{phi}_P6p"]
        .copy()
        .rename(
            index=str,
            columns={
                f"{costheta_l}_P6p": costheta_l,
...
def fold_P8p(data, costheta_k, costheta_l, phi):
    ...
    data[f"{costheta_k}_P8p"] = np.where(
        theta_l > 0.5 * pi, np.cos(pi - theta_k), data[costheta_k]
    )

    data[f"{phi}_P8p"] = np.where(data[phi] > 0.5 * pi, pi - data[phi], data[phi])
    data[f"{phi}_P8p"] = np.where(
        data[f"{phi}_P8p"] < -0.5 * pi, -pi - data[f"{phi}_P8p"], data[f"{phi}_P8p"]
    )
    data[f"{costheta_l}_P8p"] = np.where(
        theta_l > 0.5 * pi, np.cos(pi - theta_l), data[costheta_l]
    )

    return zfit.data.Data.from_pandas(
        data[f"{costheta_l}_P8p", f"{costheta_k}_P8p", f"{phi}_P8p"]
        .copy()
        .rename(
            index=str,
            columns={
...
class B2Kstll:
    FOLDS = {
        "P4p": (P4pPDF, ["FL", "AT2", "P4p"], fold_P4p),
